...
class Carrito extends ContFirestore {
  ...
  async AgregarProductoIdCarrito(id, properties) {
    ...
    if (productToUpdate === -1)
      throw new NotFoundError("El id solicitado no se encuentra");

    const arrayProductos = data.docs[productToUpdate].data();
    arrayProductos.productos.push(properties);
    await super.updateItem(data.docs[productToUpdate].id, {
  ...
  async eliminarIdProductoIdCarrito(idCarrito, idProducto) {
    ...
    const cartToFilter = data.docs.findIndex(
      (el) => el.data().id === +idCarrito
    );

    if (cartToFilter === -1)
      throw new NotFoundError("El id de carrito solicitado no se encuentra");

    const dataFiltered = data.docs[cartToFilter]
      .data()
      .productos.filter((el) => el.id !== +idProducto);

    if (data.docs[cartToFilter].data().productos.length === dataFiltered.length)
      throw new NotFoundError("El id de producto solicitado no se encuentra");

    const arrayProductos = data.docs[cartToFilter].data();
    arrayProductos.productos = dataFiltered;

    await super.updateItem(data.docs[cartToFilter].id, {
      productos: arrayProductos.productos,
    });
  }
