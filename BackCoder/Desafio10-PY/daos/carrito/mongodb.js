...
class Carrito extends ContMongoDB {
  ...
  async AgregarProductoIdCarrito(id, properties) {
    ...
    await super.updateItem(data[productToUpdate]._id, {
      productos: data[productToUpdate].productos,
    });
    return properties;
  }
  ...
  async eliminarIdProductoIdCarrito(idCarrito, idProducto) {
    ...
    const dataFiltered = data[cartToFilter].productos.filter(
      (el) => el.id !== +idProducto
    );

    if (data[cartToFilter].productos.length === dataFiltered.length)
