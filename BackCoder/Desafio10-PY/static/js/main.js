(() => {
  ...
  const renderProductos = async () => {
    ...
    data.forEach((element) => {
      ...
      div.innerHTML += `
            <img src="${element.foto}" class="card-img-top object-fit" alt="${
        element.nombre
      }">
            <div class="card-body d-flex flex-column align-items-center">
                ...
                <a href="#" id="comprar${
                  element.id
                }" class="m-1 btn btn-primary">Comprar</a>
                ${
                  admin
                    ? `<a href="#" id="modificar${element.id}" class="m-1 btn btn-primary">Modificar</a>`
                    : ""
                }
                ${
                  admin
                    ? `<a href="#" id="eliminar${element.id}" class="m-1 btn btn-primary">Eliminar</a>`
                    : ""
                }
            </div>
            `;
      ...
      if (admin) {
        div.querySelector("#modificar" + element.id).onclick = (e) => {
          ...
          div.querySelector(`#modificarform${element.id}`).onclick =
            async () => {
              const nombre = div.querySelector(
                `#modificarform-nombre${element.id}`
              ).value;
              const descripcion = div.querySelector(
                `#modificarform-descripcion${element.id}`
              ).value;
              const codigo = div.querySelector(
                `#modificarform-codigo${element.id}`
              ).value;
  ...
  const renderCarrito = async () => {
    ...
    data.forEach((element) => {
      ...
      div.innerHTML += `
        ...
        element.nombre
            ...
            <div class="card-body d-flex flex-column align-items-center">
                ...
                ${
                  admin
                    ? `<a href="#eliminarcarrito${element.id}" id="eliminarcarrito${element.id}" class="m-1 btn btn-primary">Eliminar</a>`
                    : ""
                }
            </div>
            `;
      ...
      if (admin) {
        div.querySelector("#eliminarcarrito" + element.id).onclick = async (
          e
        ) => {
          await fetch(
            `http://localhost:8080/api/carrito/1/productos/${element.id}`,
            {
              method: "DELETE",
            }
          );
